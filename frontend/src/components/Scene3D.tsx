...
const Scene3D: React.FC<Scene3DProps> = ({
  ...
  const { nodes3D, edges3D } = useMemo(() => {
    ...
    console.log('Processing graph data for 3D visualization...');
    return visualizationService.processGraphData(graphData.nodes, graphData.links || []);
  }, [graphData, visualizationService]);
  
  // Calculate optimal camera position when data changes
  useEffect(() => {
    if (nodes3D.length > 0 && controlsRef.current) {
      const { position, target } = visualizationService.calculateCameraPosition(nodes3D);
  ...
  }, [nodes3D, camera, visualizationService]);
  
  // Handle node selection
  useEffect(() => {
    if (selectedNode && controlsRef.current) {
      const selectedNode3D = nodes3D.find(n => n.id === selectedNode.id);
      if (selectedNode3D) {
